...
import me.dio.credit.system.services.impl.CreditService
...
class CreditServiceTest {
    ...
    fun `should create credit `() {
        ...
        //then
        io.mockk.verify(exactly = 1) { customerService.findById(customerId) }
        io.mockk.verify(exactly = 1) { creditRepository.save(credit) }
    ...
    fun `should not create credit when invalid day first installment`() {
        ...
        //then
        io.mockk.verify(exactly = 0) { creditRepository.save(any()) }
    }
    ...
    fun `should return list of credits for a customer`() {
        ...
        io.mockk.verify(exactly = 1) { creditRepository.findAllByCustomerId(customerId) }
    }
    ...
    fun `should return credit for a valid customer and credit code`() {
        ...
        io.mockk.verify(exactly = 1) { creditRepository.findByCreditCode(creditCode) }
    }
    ...
    fun `should throw BusinessException for invalid credit code`() {
        ...
        //then
        io.mockk.verify(exactly = 1) { creditRepository.findByCreditCode(invalidCreditCode) }
    }
